...
teste_dados_usuarios_atuais = [
    (1, 'George', 'Bluth', 'george.bluth@reqres.in'),     # usuário1
    (2, 'Janet', 'Weaver', 'janet.weaver@reqres.in'),     # usuário2
]
...
def testar_dados_usuarios(id,nome,sobrenome,email):    #função que testa o algo
    ...
    except HTTPError as http_fail:      # Para o ISTB, descobriu rodando é falha
...
# função que faz algo --> Fora do meu computador (teste de integração)
...
def ler_dados_do_csv():
    ...
    try:
        with open(nome_arquivo,newline='') as csvfile:
            ...
            next(dados) #faz pular a leitura da primeira linha
            for linha in dados:
